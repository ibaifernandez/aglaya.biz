...
- **Styling**: Tailwind v4 via Vite plugin, NOT PostCSS. **No brand value is written in this repo.** Tokens are consumed from `@aglaya/design-tokens`, pinned to a git tag in `package.json`; `src/styles/global.css` imports the canon and a Tailwind `@theme` bridge that `scripts/generate-tailwind-theme.mjs` regenerates on every install/build and that is never committed. Which tokens exist and what they are worth is state — ask `list_tokens` / `get_token` (MCP `aglaya-ds`), do not read a value off this file. How far behind this repo is: `npm run tokens:version`.
- **Fonts**: Outfit (display/headings), Inter (body), Space Mono (`--font-mono` — eyebrows, labels, form microcopy). All three are served from this origin, declared by the same package — no Google Fonts link. A family named in a `--font-*` token but backed by no `@font-face` renders as a silent fallback for every visitor who lacks it locally, and on the author's machine everything looks fine — guarded by `tests/unit/fonts-declared-are-loaded.test.ts`, which now reads the package's CSS as well as ours.
- **Consuming the brand vs copying it**: the guards in `tests/unit/brand-is-consumed-not-copied.test.ts` are cheap and can all be satisfied by a good copy. The one that cannot is `npm run tokens:mutation-proof` (CI job "Design-system mutation proof"): it moves a colour at the source, rebuilds without touching this repo, and fails if the pixel did not follow. Deliberately NOT consuming: `public/bill-capital/`, national flags, third-party client logos, vendor widgets, and Mermaid `style` directives inside `src/content/`.
...
- **Technical API:** `crm-ingestion-api.md`, in the CRM's own repo (`crm-aglaya`) — endpoints, payloads, `Idempotency-Key`. Where inside that repo it lives is the CRM's business and has moved before: ask `contrato("crm-ingestion")`, which opens it live and cites the path it read. The CRM owns this spec; it wins on interface details. The governance contract above wins on the legal/data-protection model. Its version and what it documents are state — read the file, or ask `contrato("lead-capture")` / `firmas()`. (The consent fields §3-bis live in its §10, added in v1.4.0; a note here claiming they were undocumented outlived that by five weeks.)
